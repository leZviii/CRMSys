<%@ page import="com.leviii.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.leviii.crm.workbench.domain.Tran" %>
...
<%
	...
	Map<String,String> result = (Map<String, String>) application.getAttribute("result");
	...
	for (int i = 0;i < stage.size();i++){
		...
		String stageValue = dicValue.getValue();
		String possibility = result.get(stageValue);
...
<script type="text/javascript">
			...
			success : function (data){
				$.each(data,function (index,element){
					$("#tranHistoryBody").append("<tr>")
						.append("<td>" + element.stage + "</td>")
						.append("<td>" + element.money + "</td>")
						.append("<td>" + element.possibility + "</td>")
						.append("<td>" + element.expectedDate + "</td>")
						.append("<td>" + element.createTime + "</td>")
						.append("<td>" + element.createBy + "</td>")
						.append("</tr>")
				})
			}
		})
	...
	changeIcon = function (stage,index1){
		...
		if (currentPossibility === "0"){
			...
			for (var j = point;j < <%=stage.size()%>;j++){
				if (j == index){
					$("#"+j).removeClass();
			...
			for (var k = 0;k < point;k++){
				if (k == index){
					...
					$("#"+k).removeClass();
					$("#"+k).addClass("glyphicon glyphicon-ok-circle mystage");
					$("#"+k).css("color","#90F790");
				}else {
					$("#"+k).removeClass();
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			String possibility = result.get(currentStage);

			if ("0".equals(possibility)){

				for (int i = 0;i < stage.size();i++) {
					DicValue dicValue = stage.get(i);
					String stageList = dicValue.getValue();
					String possibilityList = result.get(stageList);

					if ("0".equals(possibilityList)) {

						if (currentStage.equals(stageList)) {
		%>
		...
		<%
				...
				for (int i = 0;i < stage.size();i++){
					DicValue dicValue = stage.get(i);
					String stage1 = dicValue.getValue();
					String possibility1 = result.get(stage1);
					if (stage1.equals(currentStage)){
						index = i;
						break;
					}
				}
