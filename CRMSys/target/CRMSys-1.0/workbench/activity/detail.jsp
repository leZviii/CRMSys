...
<script type="text/javascript">
	...
	$(function(){
			...
			$.ajax({
				...
				success : function (data){
					...
					if (data.success){
						...
						html += '<h5 id="a' + data.activityRemark.id + '">' + data.activityRemark.noteContent + '</h5>';
						html += '<font color="gray">市场活动</font> <font color="gray">-</font> <b>${activity.name}</b> <small style="color: gray;" id="b' + data.activityRemark.id + '"> ' + data.activityRemark.createTime + ' 由' + data.activityRemark.createBy+ '</small>';
		...
		$("#updateRemarkBtn").click(function (){
			var activityRemarkId = $("#remarkId").val();
			$.ajax({
				...
				success : function (data){
					if (data.success){
						$("#a"+activityRemarkId).html(data.activityRemark.noteContent);
						$("#b"+activityRemarkId).html(data.activityRemark.editTime+"由"+data.activityRemark.editBy);
		...
		$.ajax({
			...
			success : function (data){
				...
				$.each(data,function (index,element){
					...
					html += '<font color="gray">市场活动</font> <font color="gray">-</font> <b>${activity.name}</b> <small style="color: gray;" id="b' + element.id + '"> ' + (element.editFlag == 0 ? element.createTime : element.editTime) + ' 由' + (element.editFlag == 0 ? element.createBy : element.editBy) + '</small>';
					html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
