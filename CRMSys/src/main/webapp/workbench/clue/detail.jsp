...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#activityName").keydown(function (event){
			if (event.keyCode === 13){
				$("#activityNotBindBody").html("");
				$.ajax({
					url : "workbench/clue/getActivityListNotBind.do",
					data : {
					...
					success : function (data){
						$.each(data,function (index,element){
							$("#activityNotBindBody").append("<tr>")
								.append("<td><input type='checkbox' name='activitySelected' value='" + element.id + "'></td>")
		...
		$("#bindBtn").click(function (){
			var $activitySelected = $("input[name=activitySelected]:checked");
			...
			if ($activitySelected.length===0){
				...
				for (var i = 0;i < $activitySelected.length;i++){
					param += "activityId="+$($activitySelected[i]).val();
					if (i < $activitySelected.length - 1){
						param += "&";
					}
				}
				// alert(param);
				$.ajax({
					...
					success : function (data){
						if (data.success){
							$("#activityName").html("");
							$("input[name=activitySelected]:checked").val("unchecked");
							showActivityList();
			...
			success : function (data){
				$.each(data,function (index,element){
					$("#activityBody").append("<tr>")
							...
							.append("<td>" + element.endDate + "</td>")
							.append("<td>" + element.owner + "</td>")
							.append("<td><a href=\"javascript:void(0);\" onclick='unbind(\"" + element.id + "\")' style=\"text-decoration: none;\"><span class=\"glyphicon glyphicon-remove\"></span>解除关联</a></td>")
							.append("</tr>")
				})
			}
